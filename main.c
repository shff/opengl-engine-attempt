...
const char *vertShader = GLSL(
  ...
  layout(location = 1) in vec2 vUV; uniform mat4 P;
  uniform mat4 V; out vec2 fUV;
...
const char *fragShader = GLSL(
  in vec2 fUV; out vec4 color; uniform sampler2D tex;
...
const char *skyVertShader = GLSL(
  out vec3 pos; out vec3 fsun; uniform mat4 P; uniform mat4 V;
...
const char *skyFragShader = GLSL(
  ...
  void main() {
    ...
    vec3 extinction = mix(exp(-exp(-((pos.y + fsun.y * 4.0) *
                                     (exp(-pos.y * 16.0) + 0.1) / 80.0) /
                                   Br) *
                              (exp(-pos.y * 16.0) + 0.1) * Kr / Br) *
                              exp(-pos.y * exp(-pos.y * 8.0) * 4.0) *
                              exp(-pos.y * 2.0) * 4.0,
                          vec3(1.0 - exp(fsun.y)) * 0.2, -fsun.y * 0.2 + 0.5);
    color.rgb = 3.0 / (8.0 * 3.14) * (1.0 + mu * mu) *
                (Kr +
                 Km * (1.0 - g * g) / (2.0 + g * g) /
...
float floorCoords[] = {
    30.0f,  -1.0f, -30.0f, 5.0f,  0.0f,  0.0f,  -30.0f, -1.0f,
    -30.0f, 0.0f,  0.0f,   0.0f,  30.0f, -1.0f, 30.0f,  5.0f,
    5.0f,   0.0f,  -30.0f, -1.0f, 30.0f, 0.0f,  5.0f,   0.0f,
};
...
void glAssert(unsigned int obj, GLenum statusType,
              void (*ivFun)(GLuint, GLenum, GLint *),
...
unsigned int loadTexture(char *filename)
{
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, (int)h.width, (int)h.height, 0,
...
unsigned int makeFramebuffer(unsigned int *renderTexture,
                             unsigned int *depthTexture, int w, int h)
{
...
entity makeEntity(scene *s, const char *vs, const char *fs, int texcount,
                  char textures[][40], void *data, unsigned int vertices,
                  unsigned int layouts, int is_framebuffer, int depth_test,
                  int w, int h)
{
  entity e = {
...
void renderEntity(entity e, matrix P, matrix V, float time)
{
  ...
  for (int i = 0; i < (e.fb ? 2 : e.texcount); i++)
    glActiveTexture(GL_TEXTURE0 + (unsigned int)i),
        glBindTexture(GL_TEXTURE_2D, e.textures[i]), glUniform1i(e.tex + i, i);
...
int main()
{
  ...
  makeEntity(&s, postVertShader, postFragShader, 0, NULL, NULL, 4, 0, 1, 0, 800,
             600);
